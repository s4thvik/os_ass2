...
void nonpremsjf(map<int, string>& names, vector<vector<int>>& data) {
    ...
    while (!pq.empty() || !wait.empty() || ind < noproc) {
        ...
        // Process the current shortest job in the priority queue for each CPU
        for (int i = 0; i < 2; i++) {  // Handling for both CPUs
            if (!pq.empty()) {
                ...
                // Record the process execution details for the current CPU
                processDetails.push_back(make_tuple(cpu[i], prNo, prPo, startTime, endTime));
            }
        }
        ...
        // Add new processes as they arrive
